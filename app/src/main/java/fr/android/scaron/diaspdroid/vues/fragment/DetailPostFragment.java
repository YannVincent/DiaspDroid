...
//    WebView texte = (WebView)convertView.findViewById(R.id.detailPostTexte);
    ImageView image;
    ...
    public void onAttach(Activity activity) {
        ...
        LOG.d(".onAttach set lister to  : " + activity);
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        try {
            ...
            if (bundle != null) {
                ...
                LOG.d(".onActivityCreated find web view with id  R.id.detailPostText");
                texte = (TextView) getActivity().findViewById(R.id.detailPostText);
                LOG.d(".onActivityCreated texte found ? " + (texte != null));
                if (texte ==null){
                    LOG.d(".onActivityCreated 'detailPostText' to create");
                    texte = new TextView(getActivity());
                }

                LOG.d(".onActivityCreated find web view with id  R.id.detailPostAvatar");
                avatar = (ImageView)getActivity().findViewById(R.id.detailPostAvatar);
                if (avatar ==null){
                    LOG.d(".onActivityCreated 'detailPostAvatar' to create");
                    avatar = new ImageView(getActivity());
                }
                LOG.d(".onActivityCreated find web view with id  R.id.detailPostUser");
                user = (TextView)getActivity().findViewById(R.id.detailPostUser);
                if (user ==null){
                    LOG.d(".onActivityCreated 'detailPostUser' to create");
                    user = new TextView(getActivity());
                }
                LOG.d(".onActivityCreated find web view with id  R.id.detailPostDatetime");
                datetime = (TextView)getActivity().findViewById(R.id.detailPostDatetime);
                if (datetime ==null){
                    LOG.d(".onActivityCreated 'detailPostDatetime' to create");
                    datetime = new TextView(getActivity());
                }
                LOG.d(".onActivityCreated find web view with id  R.id.detailPostImage");
                image = (ImageView)getActivity().findViewById(R.id.detailPostImage);
                if (image ==null){
                    LOG.d(".onActivityCreated 'detailPostImage' to create");
                    image = new ImageView(getActivity());
                }

                LOG.d(".onActivityCreated getting supportFragmentManager");
                supportFragmentManager =  listener.getSupportFragmentManager();
    ...
    public void setWebVideo(final String htmlSrc){
        ...
        try {
            LOG.d(".setWebVideo find youtube fragment with id  R.id.detailPostObjectData");
            if (supportFragmentManager==null){
                supportFragmentManager = listener.getSupportFragmentManager();
            }
            if (supportFragmentManager==null){
                LOG.e(".setWebVideo impossible to get supportFragmentManager to add Webvideo = "+htmlSrc);
                return;
            }
            FragmentTransaction sfmTx = supportFragmentManager.beginTransaction();
            LOG.d(".setWebVideo fragment 'detailPostObjectData' to create");
            WebViewSupportFragment webVideoFragment = new WebViewSupportFragment();
            LOG.d(".setWebVideo get webView from webVideoFragment");
            WebView webView = webVideoFragment.getWebView();
            if (webView==null){
                ...
                webView.loadDataWithBaseURL(null, "<html><head><meta name=\"viewport\" content=\"width=device-width; user-scalable=no; initial-scale=1.0; minimum-scale=1.0; maximum-scale=1.0; target-densityDpi=device-dpi;\"/></head><body>" + htmlSrc + "</body></html>", mimeType, encoding, "");
                webView.setVisibility(View.VISIBLE);
                webView.getSettings().setLoadWithOverviewMode(true);
                webView.getSettings().setUseWideViewPort(true);

                LOG.d(".setWebVideo create FragmentTransaction");
                LOG.d(".setWebVideo replace(R.id.detailPostObjectData with webVideoFragment in FragmentTransaction");
                sfmTx.replace(R.id.detailPostObjectData, webVideoFragment);
                LOG.d(".setWebVideo commit FragmentTransaction");
                sfmTx.commit();
            }
        }catch(Throwable thr) {
    ...
    public void setYoutubeVideo(final String urlSrc){
        ...
        try {
            ...
            if (supportFragmentManager==null){
                LOG.e(".setYoutubeVide impossible to get supportFragmentManager to add Youtube video ID = "+urlSrc);
                return;
            }
